...
const RegisterEmail = ({ nav }) => {
  const { C, inviteToken, setPendingEmail } = useTheme();
  ...
  const sendCode = async () => {
    ...
    // lost. Encoding it into the redirect URL means it's still there if
    // the magic link itself is clicked (the code path below doesn't need
    // this, since it never leaves the app at all).
    ...
    <div style={{ height: '100%', background: C.bg, display: 'flex', flexDirection: 'column', transition: 'background 0.3s' }}>
      ...
      <div style={{ flex: 1, padding: '16px 24px 24px', display: 'flex', flexDirection: 'column', gap: 24 }}>
        <div>
          ...
          <div style={{ color: C.ts, fontSize: 13, marginTop: 6, lineHeight: 1.5 }}>We'll email you a 6-digit code to enter — no password, and no link to click if your email provider blocks or pre-scans links.</div>
        </div>
        <div>
...
const RegisterEmailSent = ({ nav }) => {
  ...
  const verifyCode = async () => {
    ...
    if (trimmed.length < 6 || !pendingEmail) return;
    setVerifying(true);
    setError('');
    const { error: verifyErr } = await supabase.auth.verifyOtp({ email: pendingEmail, token: trimmed, type: 'email' });
    setVerifying(false);
    if (verifyErr) { setError(verifyErr.message); return; }
    // No manual navigation needed — a successful verifyOtp establishes a
    // real session, which fires the onAuthStateChange listener in App.jsx
    // and routes automatically, same as a clicked magic link does.
  };
    ...
    <div style={{ height: '100%', background: C.bg, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'space-between', padding: '56px 28px 44px', transition: 'background 0.3s' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 18, width: '100%' }}>
        ...
        <div style={{ textAlign: 'center' }}>
          ...
          <div style={{ color: C.ts, fontSize: 13, marginTop: 10, lineHeight: 1.6, maxWidth: 280 }}>Enter the 6-digit code we sent to <span style={{ color: C.tp, fontWeight: 600 }}>{pendingEmail}</span> below — or tap the link in that same email if you'd rather.</div>
        </div>
        <div style={{ width: '100%' }}>
          <FieldLabel>6-DIGIT CODE</FieldLabel>
          <TextField value={code} onChange={v => setCode(v.replace(/\D/g, '').slice(0, 6))} placeholder="123456" />
        </div>
        {error && (
          <div style={{ width: '100%', background: `${C.danger}18`, border: `1px solid ${C.danger}44`, borderRadius: 10, padding: '12px 14px', color: C.danger, fontSize: 13 }}>⚠️ {error}</div>
        )}
      </div>
      <div style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: 10, alignItems: 'center' }}>
        <div
          onClick={verifyCode}
          style={{ width: '100%', background: code.trim().length === 6 ? C.accent : C.surface2, borderRadius: 12, padding: '15px', textAlign: 'center', color: code.trim().length === 6 ? '#FFFFFF' : C.ts2, fontWeight: 800, fontSize: 14, fontFamily: 'Rajdhani, sans-serif', letterSpacing: 1, cursor: code.trim().length === 6 ? 'pointer' : 'default' }}
        >{verifying ? 'VERIFYING…' : 'VERIFY CODE'}</div>
        <div onClick={resendCode} style={{ color: C.ts2, fontSize: 11, textAlign: 'center', cursor: 'pointer', marginTop: 4 }}>{resent ? 'Code resent ✓' : "Didn't get it? Resend code"}</div>
        <div onClick={() => nav('register_email')} style={{ color: C.ts2, fontSize: 11, textAlign: 'center', cursor: 'pointer' }}>Use a different email address</div>
      </div>
    </div>
  );
};
