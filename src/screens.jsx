...
const CreatePost = ({ nav }) => {
  ...
  useEffect(() => {
    db.fetchMyGroups()
      .then(all => setClosedGroups(db.filterClosedGroups(all)))
        ...
        <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 10, padding: '12px 14px' }}>
          ...
          <div style={{ display: 'flex', gap: 8, marginBottom: selectedAudiences.includes('groups') ? 14 : 0 }}>
            {audienceOptions.map(opt => {
              ...
              return (
                <div key={opt.id} onClick={() => toggleAudience(opt.id)} style={{ flex: 1, padding: '10px 6px', textAlign: 'center', borderRadius: 8, cursor: 'pointer', background: active ? C.accentDim : 'transparent', border: `1.5px solid ${active ? C.accent : C.border}`, position: 'relative' }}>
