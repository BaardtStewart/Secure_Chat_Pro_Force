...
const BottomNav = ({ active, onNavigate }) => {
  ...
  const items = [
    ...
    { id: 'groups_list',  icon: '◉',  label: 'Groups',  badge: unread ? unread.groups : GROUPS_UNREAD  },
...
const CompanyFeed = ({ nav }) => {
  const { C, showGroups, refreshUnread } = useTheme();
  const [liked, setLiked] = useState({});
  const [menuPost, setMenuPost] = useState(null);
  const [posts, setPosts] = useState(null); // null = still loading
  const [error, setError] = useState('');
  const [groups, setGroups] = useState(null); // real closed groups for the shortcuts strip — was hardcoded mock data
  ...
  useEffect(() => {
    ...
    db.fetchMyGroups()
      ...
      .catch(() => { if (active) setGroups([]); }); // strip just shows the "+" tile if this fails — not worth a hard error for a shortcuts row

    db.getMyCompanyContext()
      .then(({ allMembersGroupId }) => db.markGroupRead(allMembersGroupId))
      .then(() => { if (active) refreshUnread(); })
      .catch(() => { /* best-effort — badge just won't clear this time */ });
...
const PublicFeed = ({ nav }) => {
  const { C, refreshUnread } = useTheme();
  const [liked, setLiked] = useState({});
  const [menuPost, setMenuPost] = useState(null);
...
const GroupDetail = ({ nav }) => {
  const { C, selectedGroup, refreshUnread } = useTheme();
