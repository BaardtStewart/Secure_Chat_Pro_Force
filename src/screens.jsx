...
// shared by BottomNav badges, GroupsList, and HomeFeed groups row.
...
const HOME_UNREAD = MY_GROUPS.filter(g => g.allMembers).reduce((sum, g) => sum + g.unread, 0);
// Groups badge = total unread across every group the user belongs to,
// including the platform-wide space — it's still just another group here
const GROUPS_UNREAD = MY_GROUPS.reduce((sum, g) => sum + g.unread, 0);

const BottomNav = ({ active, onNavigate }) => {
  const { C } = useTheme();
  const items = [
    { id: 'home_feed',    icon: '⊞',  label: 'Home',     badge: HOME_UNREAD   },
    { id: 'groups_list',  icon: '◉',  label: 'Groups',   badge: GROUPS_UNREAD },
    { id: null,           icon: '···', label: 'Coming',   future: true         },
  ...
  return (
    <div style={{
      ...
      {items.map((item, i) => (
        <div
          key={i}
          onClick={() => !item.future && item.id && onNavigate(item.id)}
          style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3, cursor: item.future ? 'default' : 'pointer', minHeight: 44, justifyContent: 'center' }}
        >
          {item.future ? (
            <>
              <div style={{ width: 28, height: 28, borderRadius: 8, border: `1.5px dashed ${C.border}`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <span style={{ fontSize: 11, color: C.ts2, letterSpacing: 1 }}>···</span>
              </div>
              <span style={{ fontSize: 10, color: C.ts2, letterSpacing: 0.3 }}>Coming</span>
            </>
          ) : (
            <>
...
const InviteGeneratorCard = ({ title, expiryLabel = '48 hours', extraNote }) => {
  const { C } = useTheme();
  ...
  return (
    <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 12, padding: '18px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 14 }}>
      ...
      <div style={{ width: 150, height: 150, borderRadius: 14, background: C.surface2, border: `2px solid ${C.accent}44`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 70 }}>▦</div>
      ...
      <div style={{ display: 'flex', gap: 8, width: '100%' }}>
        <div onClick={() => setCopied(true)} style={{ flex: 1, padding: '12px', textAlign: 'center', borderRadius: 8, background: C.accent, color: '#FFFFFF', fontSize: 13, fontWeight: 800, cursor: 'pointer' }}>{copied ? 'Copied ✓' : 'Copy Link'}</div>
        <div onClick={() => setShared(true)} style={{ flex: 1, padding: '12px', textAlign: 'center', borderRadius: 8, border: `1px solid ${C.border}`, color: C.tp, fontSize: 13, cursor: 'pointer' }}>{shared ? 'Shared ✓' : '↗ Share'}</div>
      </div>
    </div>
  );
};
...
const InviteGate = ({ nav }) => {
  const { C } = useTheme();
  const [scanState, setScanState] = useState('idle'); // 'idle' | 'verified'
  return (
    <div style={{ height: '100%', background: C.bg, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'space-between', padding: '56px 28px 44px', transition: 'background 0.3s' }}>
      ...
      {scanState === 'idle' ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 18 }}>
          ...
          <div style={{ textAlign: 'center' }}>
            ...
            <div style={{ color: C.ts, fontSize: 13, marginTop: 10, lineHeight: 1.6, maxWidth: 260 }}>This invite was issued by Pro Force Secure and is valid. Let's set up your account.</div>
          </div>
        </div>
      )}
      <div style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: 10 }}>
        {scanState === 'idle' ? (
          <>
            <div onClick={() => setScanState('verified')} style={{ background: C.accent, borderRadius: 12, padding: '16px', textAlign: 'center', color: '#FFFFFF', fontWeight: 800, fontSize: 14, fontFamily: 'Rajdhani, sans-serif', letterSpacing: 1.5, cursor: 'pointer', boxShadow: `0 4px 24px ${hexToRgba(C.accent, 0.4)}` }}>📷  SCAN INVITE QR CODE</div>
            <div onClick={() => nav('register_verify_choice')} style={{ background: 'transparent', border: `1px solid ${C.border}`, borderRadius: 12, padding: '14px', textAlign: 'center', color: C.ts, fontSize: 14, cursor: 'pointer' }}>I have an invite link</div>
...
const RegisterVerifyChoice = ({ nav }) => {
  ...
  return (
    <div style={{ height: '100%', background: C.bg, display: 'flex', flexDirection: 'column', transition: 'background 0.3s' }}>
      <TopBar title="" back onBack={() => nav('invite_gate')} />
      <div style={{ flex: 1, padding: '16px 24px 24px', display: 'flex', flexDirection: 'column', gap: 24 }}>
        ...
        <div style={{ background: C.surface, borderRadius: 10, padding: '12px 14px', border: `1px solid ${C.border}`, display: 'flex', gap: 10 }}>
          ...
          <span style={{ color: C.ts, fontSize: 12, lineHeight: 1.5 }}>Either method confirms you're a real person before you can join. You can change your registered contact details later from Edit Profile.</span>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div style={{ height: '100%', background: C.bg, display: 'flex', flexDirection: 'column', transition: 'background 0.3s' }}>
      ...
      <div style={{ flex: 1, padding: '16px 24px 24px', display: 'flex', flexDirection: 'column', gap: 24 }}>
        ...
        <div onClick={() => nav('register_email_sent')} style={{ background: C.accent, borderRadius: 10, padding: '15px', textAlign: 'center', color: '#FFFFFF', fontWeight: 800, fontSize: 14, fontFamily: 'Rajdhani, sans-serif', letterSpacing: 1, cursor: 'pointer', boxShadow: `0 4px 20px ${hexToRgba(C.accent, 0.35)}` }}>SEND MAGIC LINK</div>
        <div style={{ background: C.surface, borderRadius: 10, padding: '12px 14px', border: `1px solid ${C.border}`, display: 'flex', gap: 10 }}>
          <span style={{ color: C.accent, flexShrink: 0 }}>🔒</span>
    ...
    <div style={{ height: '100%', background: C.bg, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'space-between', padding: '56px 28px 44px', transition: 'background 0.3s' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 18 }}>
        ...
        <div style={{ textAlign: 'center' }}>
          ...
          <div style={{ color: C.ts, fontSize: 13, marginTop: 10, lineHeight: 1.6, maxWidth: 260 }}>We've sent a magic link to your email. Open it on this device to continue — the link expires in 15 minutes.</div>
        </div>
      </div>
      <div style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: 10 }}>
        <div onClick={() => nav('register_profile')} style={{ background: C.success, borderRadius: 12, padding: '16px', textAlign: 'center', color: '#FFFFFF', fontWeight: 800, fontSize: 14, fontFamily: 'Rajdhani, sans-serif', letterSpacing: 1.5, cursor: 'pointer', boxShadow: '0 4px 24px rgba(34,197,94,0.35)' }}>✓ SIMULATE: I CLICKED THE LINK</div>
        <div style={{ color: C.ts2, fontSize: 11, textAlign: 'center' }}>Didn't get it? Check spam, or go back to try again.</div>
      </div>
    </div>
  );
};
  ...
  return (
    <div style={{ height: '100%', background: C.bg, display: 'flex', flexDirection: 'column', transition: 'background 0.3s' }}>
      ...
      <div style={{ flex: 1, padding: '0 24px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 18 }}>
        ...
        <div onClick={() => nav('home_feed')} style={{ background: C.accent, borderRadius: 10, padding: '15px', textAlign: 'center', color: '#FFFFFF', fontWeight: 800, fontSize: 14, cursor: 'pointer', fontFamily: 'Rajdhani, sans-serif', letterSpacing: 1, boxShadow: `0 4px 20px ${hexToRgba(C.accent, 0.35)}`, marginBottom: 24 }}>COMPLETE SETUP →</div>
      </div>
    </div>
  );
};
...
// ─── SCREEN 5: Home Feed ───────────────────────────────────────────────────────
const HomeFeed = ({ nav }) => {
  const { C, showGroups } = useTheme();
  const [liked, setLiked] = useState({});
  const [menuPost, setMenuPost] = useState(null);
  // Use module-level MY_GROUPS so group row is consistent with GroupsList
  const groups = MY_GROUPS.map(g => ({ init: g.init, label: g.label.replace(' ', '\n') }));
  const posts = [
    { id: 'p1', init: 'S', name: 'Sipho Mokoena', group: 'Sandton CBD Site', time: '8 min ago', body: 'Perimeter check complete. North gate secured. All clear on east corridor.', hasImg: true, badge: 'SITE UPDATE', badgeColor: C.accent, likes: 4, comments: 2 },
    { id: 'p2', init: 'T', name: 'Thabo Khumalo', group: 'Night Shift Alpha', time: '42 min ago', body: 'Shift handover complete. Incident log updated. Relieving team has been briefed on the active situation at Gate 3.', hasImg: false, badge: 'HANDOVER', badgeColor: C.alert, likes: 7, comments: 5 },
    { id: 'p3', init: 'N', name: 'Naledi Radebe', group: 'All Members', time: '1h ago', body: 'Reminder: SOP update briefing tomorrow 07:00 at HQ. Attendance mandatory for all supervisors.', hasImg: false, badge: 'ANNOUNCEMENT', badgeColor: '#8B6914', likes: 12, comments: 3 },
  ];
  ...
  return (
    <div style={{ height: '100%', background: C.bg, display: 'flex', flexDirection: 'column', position: 'relative', transition: 'background 0.3s' }}>
        ...
        {showGroups && (
          <div style={{ padding: '14px 0 12px', borderBottom: `1px solid ${C.border}` }}>
            <div style={{ display: 'flex', gap: 14, paddingLeft: 16, overflowX: 'auto' }}>
              {groups.map(g => (
                <div key={g.init} onClick={() => nav('group_detail')} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6, cursor: 'pointer', flexShrink: 0 }}>
                  <div style={{ width: 54, height: 54, borderRadius: '50%', background: C.avatarBg, border: `2.5px solid ${C.accent}`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: C.accent, fontSize: 13, fontWeight: 800, fontFamily: 'Rajdhani, sans-serif' }}>{g.init}</div>
                  <span style={{ color: C.ts, fontSize: 10, textAlign: 'center', whiteSpace: 'pre', lineHeight: 1.2 }}>{g.label}</span>
                </div>
              ))}
        ...
        {posts.map(p => (
          <div key={p.id} style={{ padding: '14px 16px', borderBottom: `1px solid ${C.border}` }}>
            <div style={{ display: 'flex', gap: 10, marginBottom: 10, alignItems: 'flex-start' }}>
              <Avatar letter={p.init} />
              <div style={{ flex: 1 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                  <span style={{ color: C.tp, fontSize: 14, fontWeight: 700 }}>{p.name}</span>
                  <Badge color={p.badgeColor}>{p.badge}</Badge>
                </div>
                <div style={{ color: C.ts, fontSize: 11, marginTop: 2 }}>{p.group} · {p.time}</div>
              </div>
              <IconTap onClick={() => setMenuPost(p)}><span style={{ color: C.ts2, fontSize: 20, lineHeight: 1 }}>⋯</span></IconTap>
            </div>
            <div style={{ color: C.tp, fontSize: 14, lineHeight: 1.55, marginBottom: p.hasImg ? 12 : 0 }}>{p.body}</div>
            {p.hasImg && <div style={{ height: 150, borderRadius: 10, background: C.surface2, display: 'flex', alignItems: 'center', justifyContent: 'center', color: C.ts2, fontSize: 13, border: `1px solid ${C.border}` }}>📷 Site photo attached · tap to view</div>}
            <div style={{ display: 'flex', gap: 4, marginTop: 12, alignItems: 'center' }}>
              <div onClick={() => toggleLike(p.id)} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '6px 10px', borderRadius: 8, cursor: 'pointer', color: liked[p.id] ? C.accent : C.ts, fontSize: 13, fontWeight: liked[p.id] ? 700 : 400 }}>
                <span>{liked[p.id] ? '👍🏽' : '👍'}</span>{p.likes + (liked[p.id] ? 1 : 0)}
              </div>
              <div onClick={() => nav('post_detail')} style={{ padding: '6px 10px', borderRadius: 8, cursor: 'pointer', color: C.ts, fontSize: 13 }}>💬 {p.comments}</div>
      ...
      <SheetOverlay visible={!!menuPost} onClose={() => setMenuPost(null)} anchor="bottom">
        <div style={{ color: C.ts, fontSize: 11, letterSpacing: 1, marginBottom: 10 }}>POST OPTIONS</div>
        {['📋 Copy link', '↗ Share externally', '🚩 Report post'].map(label => (
          <div key={label} onClick={() => setMenuPost(null)} style={{ padding: '13px 4px', color: C.tp, fontSize: 14, cursor: 'pointer', borderBottom: `1px solid ${C.border}` }}>{label}</div>
        ))}
      </SheetOverlay>

      <BottomNav active="home_feed" onNavigate={nav} />
    </div>
  );
};
...
const PostDetail = ({ nav }) => {
  ...
  const [comments, setComments] = useState([
    { init: 'T', name: 'Thabo K', body: 'Copy that. Will monitor Camera 4 closely on next round.', time: '5 min ago' },
    { init: 'N', name: 'Naledi R', body: 'Admin notified of camera fault. Technician arranged for tomorrow.', time: '2 min ago' },
  ]);

  const sendComment = () => {
    const trimmed = comment.trim();
    if (!trimmed) return;
    setComments(prev => [...prev, { init: 'Y', name: 'You', body: trimmed, time: 'Just now' }]);
    setComment('');
  };

  return (
    <div style={{ height: '100%', background: C.bg, display: 'flex', flexDirection: 'column', position: 'relative', transition: 'background 0.3s' }}>
      <TopBar title="Post" back onBack={() => nav('home_feed')} />
      <div style={{ flex: 1, overflowY: 'auto', paddingBottom: 72 }}>
        <div style={{ padding: 16 }}>
          <div style={{ display: 'flex', gap: 10, marginBottom: 12 }}>
            <Avatar letter="S" />
            <div>
              <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}><span style={{ color: C.tp, fontSize: 14, fontWeight: 700 }}>Sipho Mokoena</span><Badge>SITE UPDATE</Badge></div>
              <div style={{ color: C.ts, fontSize: 11, marginTop: 2 }}>Sandton CBD Site · 8 min ago</div>
            </div>
          </div>
          <div style={{ color: C.tp, fontSize: 14, lineHeight: 1.6, marginBottom: 12 }}>Perimeter check complete. North gate secured. All clear on east corridor. Camera 4 was temporarily offline but has been reset — all feeds now active across all checkpoints.</div>
          <div style={{ height: 180, borderRadius: 10, background: C.surface2, border: `1px solid ${C.border}`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: C.ts2, fontSize: 13, marginBottom: 14 }}>📷 Site photo</div>
          <div style={{ display: 'flex', gap: 4, padding: '6px 0', borderBottom: `1px solid ${C.border}` }}>
            <div onClick={() => setLiked(l => !l)} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '6px 10px', borderRadius: 8, cursor: 'pointer', color: liked ? C.accent : C.ts, fontSize: 13, fontWeight: liked ? 700 : 400 }}>{liked ? '👍🏽' : '👍'} {4 + (liked ? 1 : 0)} likes</div>
            <div style={{ padding: '6px 10px', color: C.ts, fontSize: 13 }}>💬 {comments.length} comments</div>
          </div>
        </div>
        <div style={{ padding: '0 16px' }}>
          ...
          {comments.map((c, i) => (
            <div key={i} style={{ display: 'flex', gap: 10, marginBottom: 16 }}>
              <Avatar letter={c.init} size={34} />
              <div style={{ flex: 1 }}>
                <div style={{ background: C.surface, borderRadius: 10, padding: '10px 12px', border: `1px solid ${C.border}` }}>
                  <div style={{ color: C.accent, fontSize: 12, fontWeight: 700, marginBottom: 4 }}>{c.name}</div>
                  <div style={{ color: C.tp, fontSize: 13, lineHeight: 1.45 }}>{c.body}</div>
                </div>
                <div style={{ color: C.ts2, fontSize: 11, marginTop: 4, paddingLeft: 4 }}>{c.time}</div>
              </div>
            </div>
          ))}
        </div>
      ...
      <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, background: C.surface, borderTop: `1px solid ${C.border}`, padding: '10px 14px', display: 'flex', gap: 10, alignItems: 'center' }}>
        ...
        <div onClick={sendComment} style={{ width: 36, height: 36, borderRadius: '50%', background: comment.trim() ? C.accent : C.surface2, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: comment.trim() ? 'pointer' : 'default', flexShrink: 0, transition: 'background 0.15s' }}>
          <span style={{ color: comment.trim() ? '#FFFFFF' : C.ts2, fontSize: 16, fontWeight: 900 }}>↑</span>
        </div>
      </div>
    </div>
  );
...
const CreatePost = ({ nav }) => {
  const { C } = useTheme();
  const [vis, setVis] = useState('group');
  const [body, setBody] = useState('');
  const [pickerOpen, setPickerOpen] = useState(false);
  const myGroups = [
    { init: 'SC', name: 'Sandton CBD Site' }, { init: 'NA', name: 'Night Shift Alpha' },
    { init: 'AM', name: 'All Members' }, { init: 'VD', name: 'VIP Detail – June' },
  ];
  const [selectedGroup, setSelectedGroup] = useState(myGroups[0]);
  const [attachments, setAttachments] = useState([]);
  const addAttachment = type => setAttachments(a => [...a, { id: Date.now(), type }]);
  ...
  return (
    <div style={{ height: '100%', background: C.bg, display: 'flex', flexDirection: 'column', transition: 'background 0.3s' }}>
      <div style={{ display: 'flex', alignItems: 'center', padding: '48px 16px 14px', background: C.surface, borderBottom: `1px solid ${C.border}`, justifyContent: 'space-between', gap: 12 }}>
        <span onClick={() => nav('home_feed')} style={{ color: C.ts, fontSize: 14, cursor: 'pointer' }}>Cancel</span>
        <span style={{ color: C.tp, fontSize: 16, fontWeight: 700, fontFamily: 'Rajdhani, sans-serif', letterSpacing: 1 }}>New Post</span>
        <div onClick={() => nav('home_feed')} style={{ background: C.accent, borderRadius: 20, padding: '7px 18px', color: '#FFFFFF', fontWeight: 700, fontSize: 13, cursor: 'pointer' }}>POST</div>
      </div>
      <div style={{ flex: 1, overflowY: 'auto', padding: 16, display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        {attachments.length > 0 && (
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            {attachments.map(a => (
              <div key={a.id} style={{ display: 'flex', alignItems: 'center', gap: 6, background: C.surface2, border: `1px solid ${C.border}`, borderRadius: 8, padding: '6px 10px' }}>
                <span style={{ fontSize: 14 }}>{a.type === 'photo' ? '📷' : a.type === 'reel' ? '🎥' : a.type === 'location' ? '📍' : '📎'}</span>
                <span style={{ color: C.ts, fontSize: 12 }}>{a.type === 'photo' ? 'Photo attached' : a.type === 'reel' ? 'Reel attached' : a.type === 'location' ? 'Sandton CBD' : 'File attached'}</span>
                <span onClick={() => removeAttachment(a.id)} style={{ color: C.danger, fontSize: 13, cursor: 'pointer', marginLeft: 4 }}>✕</span>
              </div>
            ))}
          </div>
        )}

        <div onClick={() => setPickerOpen(true)} style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 10, padding: '12px 14px', cursor: 'pointer' }}>
          <div style={{ color: C.ts, fontSize: 11, letterSpacing: 1, marginBottom: 10 }}>POST TO GROUP</div>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <div style={{ width: 30, height: 30, borderRadius: 8, background: C.avatarBg, display: 'flex', alignItems: 'center', justifyContent: 'center', color: C.accent, fontSize: 12, fontWeight: 700 }}>{selectedGroup.init}</div>
              <span style={{ color: C.tp, fontSize: 14 }}>{selectedGroup.name}</span>
            </div>
            <span style={{ color: C.accent, fontSize: 13 }}>Change ›</span>
          </div>
        </div>

        <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 10, padding: '12px 14px' }}>
          <div style={{ color: C.ts, fontSize: 11, letterSpacing: 1, marginBottom: 10 }}>VISIBILITY</div>
          <div style={{ display: 'flex', gap: 8 }}>
            {[['group', '🔒 Group Only'], ['all', '👁 All Members']].map(([id, label]) => (
              <div key={id} onClick={() => setVis(id)} style={{ flex: 1, padding: '10px', textAlign: 'center', borderRadius: 8, cursor: 'pointer', background: vis === id ? C.accentDim : 'transparent', border: `1.5px solid ${vis === id ? C.accent : C.border}`, color: vis === id ? C.accent : C.ts, fontSize: 13, fontWeight: vis === id ? 700 : 400 }}>{label}</div>
            ))}
          </div>
        </div>
        ...
        <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 10, padding: '12px 14px', display: 'flex', gap: 10 }}>
          ...
          <span style={{ color: C.ts, fontSize: 12, lineHeight: 1.4 }}>This post will be end-to-end encrypted. Only group members can read it.</span>
        </div>
      </div>
      <div style={{ background: C.surface, borderTop: `1px solid ${C.border}`, padding: '12px 20px 20px', display: 'flex', gap: 28, alignItems: 'center' }}>
        {[['📷', 'Photo', 'photo'], ['🎥', 'Reel', 'reel'], ['📍', 'Location', 'location'], ['📎', 'Attach', 'file']].map(([icon, label, type]) => (
          <div key={label} onClick={() => addAttachment(type)} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4, cursor: 'pointer' }}>
            <span style={{ fontSize: 22 }}>{icon}</span>
            <span style={{ color: C.ts2, fontSize: 10 }}>{label}</span>
          </div>
        ))}
      </div>

      <SheetOverlay visible={pickerOpen} onClose={() => setPickerOpen(false)} anchor="bottom">
        <div style={{ color: C.ts, fontSize: 11, letterSpacing: 1, marginBottom: 10 }}>POST TO WHICH GROUP?</div>
        {myGroups.map(g => (
          <div key={g.name} onClick={() => { setSelectedGroup(g); setPickerOpen(false); }} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 4px', borderBottom: `1px solid ${C.border}`, cursor: 'pointer' }}>
            <div style={{ width: 36, height: 36, borderRadius: 10, background: C.avatarBg, display: 'flex', alignItems: 'center', justifyContent: 'center', color: C.accent, fontSize: 12, fontWeight: 700 }}>{g.init}</div>
            <span style={{ color: C.tp, fontSize: 14, flex: 1 }}>{g.name}</span>
            {g.name === selectedGroup.name && <span style={{ color: C.accent, fontSize: 16 }}>✓</span>}
          </div>
        ))}
      </SheetOverlay>
    </div>
...
const GroupsList = ({ nav }) => {
  ...
  const list = tab === 'mine' ? MY_GROUPS : DISCOVER_GROUPS;
  return (
    <div style={{ height: '100%', background: C.bg, display: 'flex', flexDirection: 'column', position: 'relative', transition: 'background 0.3s' }}>
      <TopBar title="Groups" right={<IconTap onClick={() => nav('create_group')}><div style={{ width: 30, height: 30, borderRadius: 8, background: C.accentDim, border: `1px solid ${C.accent}44`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: C.accent, fontSize: 20 }}>+</div></IconTap>} />
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '12px 16px', display: 'flex', flexDirection: 'column', gap: 8, paddingBottom: 80 }}>
        {list.length === 0 ? (
          ...
          <div key={g.label} onClick={() => nav('group_detail')} style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 12, padding: '14px', display: 'flex', gap: 12, alignItems: 'center', cursor: 'pointer' }}>
            <div style={{ width: 50, height: 50, borderRadius: 12, background: C.avatarBg, display: 'flex', alignItems: 'center', justifyContent: 'center', color: C.accent, fontSize: 14, fontWeight: 800, fontFamily: 'Rajdhani, sans-serif', flexShrink: 0 }}>{g.init}</div>
            <div style={{ flex: 1 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 5 }}>
                <span style={{ color: C.tp, fontSize: 14, fontWeight: 700 }}>{g.label}</span>
                {tab === 'mine' && g.unread > 0 && <div style={{ width: 18, height: 18, borderRadius: 9, background: C.accent, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, color: '#FFFFFF', fontWeight: 800 }}>{g.unread}</div>}
              </div>
              <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                <span style={{ color: C.ts, fontSize: 12 }}>👥 {g.members}</span>
                <Badge color={g.vis === 'Public' ? C.alert : C.ts}>{g.vis}</Badge>
                {g.scope === 'platform' && <Badge color={C.accent}>🌐 ALL COMPANIES</Badge>}
              </div>
            </div>
            {tab === 'discover' ? (
              <div onClick={e => e.stopPropagation()} style={{ padding: '7px 14px', borderRadius: 20, background: C.accent, color: '#FFFFFF', fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>Join</div>
            ) : (
              <span style={{ color: C.ts2, fontSize: 18 }}>›</span>
            )}
          </div>
...
const GroupDetail = ({ nav }) => {
  ...
  return (
    <div style={{ height: '100%', background: C.bg, display: 'flex', flexDirection: 'column', position: 'relative', transition: 'background 0.3s' }}>
      <div style={{ background: C.headerBg, padding: '48px 16px 16px', position: 'relative' }}>
        <IconTap onClick={() => nav('groups_list')}><span style={{ position: 'absolute', top: 10, left: 4, color: C.tp, fontSize: 22 }}>←</span></IconTap>
        <div style={{ display: 'flex', gap: 14, alignItems: 'flex-end' }}>
          <div style={{ width: 56, height: 56, borderRadius: 14, background: C.avatarBg, border: `2px solid ${C.accent}`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: C.accent, fontWeight: 800, fontSize: 15, fontFamily: 'Rajdhani, sans-serif' }}>SC</div>
          <div>
            <div style={{ color: C.tp, fontSize: 18, fontWeight: 800, fontFamily: 'Rajdhani, sans-serif', letterSpacing: 1 }}>Sandton CBD Site</div>
            <div style={{ display: 'flex', gap: 8, marginTop: 4, alignItems: 'center' }}>
              <span style={{ color: C.ts, fontSize: 12 }}>👥 14 members</span><Badge>Members Only</Badge>
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', paddingBottom: 70 }}>
        {tab === 'posts' && [{ init: 'S', name: 'Sipho M', time: '8 min ago', body: 'Perimeter check complete. All clear on all sectors.', img: true }, { init: 'T', name: 'Thabo K', time: '1h ago', body: 'Camera 4 back online. All feeds stable.' }].map((p, i) => (
          <div key={i} onClick={() => nav('post_detail')} style={{ padding: '14px 16px', borderBottom: `1px solid ${C.border}`, cursor: 'pointer' }}>
            <div style={{ display: 'flex', gap: 10, marginBottom: 10 }}><Avatar letter={p.init} size={36} /><div><div style={{ color: C.tp, fontSize: 13, fontWeight: 700 }}>{p.name}</div><div style={{ color: C.ts, fontSize: 11 }}>{p.time}</div></div></div>
            <div style={{ color: C.tp, fontSize: 14, lineHeight: 1.5 }}>{p.body}</div>
            {p.img && <div style={{ height: 110, borderRadius: 8, background: C.surface2, marginTop: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', color: C.ts2, fontSize: 12, border: `1px solid ${C.border}` }}>📷 Site photo</div>}
          </div>
        ))}
        {tab === 'members' && (
          <div style={{ padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: 14 }}>
            {[{ init: 'N', name: 'Naledi R', role: 'Group Admin', admin: true }, { init: 'S', name: 'Sipho M', role: 'Site Supervisor' }, { init: 'T', name: 'Thabo K', role: 'Guard · North Sector' }].map((m, i) => (
              <div key={i} style={{ display: 'flex', gap: 12, alignItems: 'center' }}><Avatar letter={m.init} /><div style={{ flex: 1 }}><div style={{ color: C.tp, fontSize: 14, fontWeight: 600 }}>{m.name}</div><div style={{ color: C.ts, fontSize: 12 }}>{m.role}</div></div>{m.admin && <Badge color={C.alert}>ADMIN</Badge>}</div>
            ))}
        ...
        {tab === 'about' && (
          <div style={{ padding: 16 }}>
            {[['Group', 'Sandton CBD Site'], ['Created by', 'Naledi R'], ['Visibility', 'Members Only'], ['Location', 'Sandton CBD, Johannesburg'], ['Created', '3 June 2025']].map(([k, v]) => (
            ...
            <div onClick={() => setConfirm({ title: 'Leave this group?', message: 'You will stop receiving updates from Sandton CBD Site. You can be re-invited by an existing member at any time.', confirmLabel: 'Leave Group', onConfirm: () => nav('groups_list') })} style={{ marginTop: 20, padding: '12px', textAlign: 'center', borderRadius: 10, border: `1px solid ${C.danger}44`, color: C.danger, fontSize: 14, cursor: 'pointer' }}>Leave Group</div>
            <div onClick={() => setConfirm({ title: 'Delete this group?', message: 'All posts, comments, and media in Sandton CBD Site will be permanently removed for all 14 members. This cannot be undone.', confirmLabel: 'Delete Group', onConfirm: () => nav('groups_list') })} style={{ marginTop: 10, padding: '12px', textAlign: 'center', borderRadius: 10, border: `1px solid ${C.danger}44`, color: C.danger, fontSize: 14, cursor: 'pointer' }}>Delete Group</div>
          </div>
        )}
      </div>
  ...
  return (
    <div style={{ height: '100%', background: C.bg, display: 'flex', flexDirection: 'column', transition: 'background 0.3s' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '16px 16px 0', display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <div onClick={() => nav('group_detail')} style={{ background: C.accent, borderRadius: 10, padding: '15px', textAlign: 'center', color: '#FFFFFF', fontWeight: 800, fontSize: 14, cursor: 'pointer', fontFamily: 'Rajdhani, sans-serif', letterSpacing: 1, boxShadow: `0 4px 20px ${hexToRgba(C.accent, 0.35)}`, marginBottom: 24 }}>CREATE GROUP</div>
      </div>
    </div>
  );
};
...
const ProfileView = ({ nav }) => {
  ...
  return (
    <div style={{ height: '100%', background: C.bg, display: 'flex', flexDirection: 'column', position: 'relative', transition: 'background 0.3s' }}>
      <div style={{ background: C.surface, borderBottom: `1px solid ${C.border}`, padding: '48px 16px 22px' }}>
        ...
        <div style={{ display: 'flex', gap: 16 }}>
          <div style={{ position: 'relative' }}>
            <Avatar letter="Y" size={76} />
            <AvatarUploadBadge onClick={() => nav('edit_profile')} size={24} />
          </div>
          <div style={{ flex: 1 }}>
            <div style={{ color: C.tp, fontSize: 18, fontWeight: 800, fontFamily: 'Rajdhani, sans-serif', letterSpacing: 0.5 }}>Yusuf Plaatjies</div>
            <div style={{ color: C.ts, fontSize: 12, marginTop: 3 }}>+27 82 *** **** · Verified ✓</div>
            <div style={{ color: C.ts, fontSize: 13, marginTop: 5, lineHeight: 1.3 }}>Field Supervisor · Western Cape Sites</div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', paddingBottom: 72 }}>
        ...
        <div style={{ padding: '0 16px' }}>
          <div onClick={() => nav('login_screen')} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '13px 0', borderBottom: `1px solid ${C.border}`, cursor: 'pointer', minHeight: 44 }}>
            <span style={{ fontSize: 18 }}>🚪</span>
            <span style={{ flex: 1, color: C.tp, fontSize: 14 }}>Sign Out</span>
            <span style={{ color: C.ts2 }}>›</span>
          </div>
        </div>
        ...
        <div style={{ padding: '20px 0 16px', borderBottom: `1px solid ${C.border}` }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 16 }}>
            <div style={{ flex: 1 }}>
              ...
              <div style={{ color: C.ts, fontSize: 12, marginTop: 3 }}>{showGroups ? 'Groups row appears at the top of your Home Feed' : 'Home Feed shows posts only — maximum reading space'}</div>
            </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '16px 16px 24px', display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <div>
          ...
          <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 10, padding: '12px 14px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              <div style={{ color: C.tp, fontSize: 13, fontWeight: 600 }}>Sent to Sipho M.</div>
              <div style={{ color: C.ts, fontSize: 11 }}>2 days ago</div>
            </div>
            <Badge color={C.ts}>USED</Badge>
          </div>
        </div>
      </div>
    </div>
  ...
  return (
    <div style={{ height: '100%', background: C.bg, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'space-between', padding: '56px 28px 44px', transition: 'background 0.3s' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 24 }}>
        ...
        {state === 'success' && (
          <div onClick={() => nav('home_feed')} style={{ background: C.success, borderRadius: 12, padding: '14px 40px', color: '#FFF', fontWeight: 800, fontSize: 14, cursor: 'pointer', fontFamily: 'Rajdhani, sans-serif', letterSpacing: 1 }}>CONTINUE →</div>
        )}
      </div>
...
const LoginAppCode = ({ nav }) => {
  ...
  const addDigit = d => {
    ...
    setDigits(next);
    if (next.length === 6) {
      if (next.join('') === '123456') { setTimeout(() => nav('home_feed'), 400); }
      ...
      <div style={{ flex: 1, padding: '8px 24px 32px', display: 'flex', flexDirection: 'column', gap: 24 }}>
        ...
        {step === 'success' && (
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 20, textAlign: 'center' }}>
            ...
            <div onClick={() => nav('home_feed')} style={{ background: C.accent, borderRadius: 10, padding: '15px 40px', color: '#FFFFFF', fontWeight: 800, fontSize: 14, cursor: 'pointer', fontFamily: 'Rajdhani, sans-serif', letterSpacing: 1, boxShadow: `0 4px 20px ${hexToRgba(C.accent, 0.35)}` }}>CONTINUE TO APP →</div>
          </div>
        )}
      </div>
    </div>
...
const AdminDashboard = ({ nav }) => {
  ...
  return (
    <div style={{ height: '100%', background: C.bg, display: 'flex', flexDirection: 'column', transition: 'background 0.3s' }}>
      <TopBar title="Admin Dashboard" back onBack={() => nav('home_feed')} right={<Badge color={C.alert}>ADMIN</Badge>} />
...
const SCREENS = {
  ...
  register_profile: RegisterProfile, home_feed: HomeFeed, post_detail: PostDetail,
...
// ─── Production exports ────────────────────────────────────────────────────────
export { SCREENS, ThemeCtx, DARK, LIGHT };
