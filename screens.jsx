...
const BottomNav = ({ active, onNavigate }) => {
  const { C } = useTheme();
  const items = [
    { id: 'company_feed', icon: '🏢', label: 'Company', badge: COMPANY_UNREAD },
    { id: 'groups_list',  icon: '◉',  label: 'Groups',  badge: GROUPS_UNREAD  },
    { id: 'public_feed',  icon: '🌐', label: 'Public',  badge: PUBLIC_UNREAD  },
...
const CompanyFeed = ({ nav }) => {
  ...
  const [error, setError] = useState('');
  const groups = MY_GROUPS.filter(g => g.scope === 'company').map(g => ({ init: g.init, label: g.label.replace(' ', '\n') }));
        ...
        {posts && posts.map(p => (
          <div key={p.id} style={{ padding: '14px 16px', borderBottom: `1px solid ${C.border}` }}>
            ...
            <div style={{ display: 'flex', gap: 4, marginTop: 12, alignItems: 'center' }}>
              ...
              <div onClick={() => nav('post_detail')} style={{ padding: '6px 10px', borderRadius: 8, cursor: 'pointer', color: C.ts, fontSize: 13 }}>💬 {p.comment_count}</div>
...
const CreatePost = ({ nav }) => {
  const { C, companyName } = useTheme();
  const [audience, setAudience] = useState('company'); // 'company' | 'groups' | 'public'
  const [body, setBody] = useState('');
  const [closedGroups, setClosedGroups] = useState(null); // real groups with real ids, fetched on mount
  ...
  useEffect(() => {
    db.fetchMyGroups()
      .then(all => {
        ...
        if (closed.length) setSelectedGroupIds([closed[0].id]);
      })
  ...
  const canPost = body.trim() && (audience !== 'groups' || selectedGroupIds.length > 0) && !posting;
  const submit = async () => {
    if (!canPost) return;
    setPosting(true);
    setError('');
    try {
      ...
      nav(audience === 'company' ? 'company_feed' : audience === 'public' ? 'public_feed' : 'groups_list');
    } catch (err) {
    ...
    <div style={{ height: '100%', background: C.bg, display: 'flex', flexDirection: 'column', transition: 'background 0.3s' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: 16, display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 10, padding: '12px 14px' }}>
          <div style={{ color: C.ts, fontSize: 11, letterSpacing: 1, marginBottom: 10 }}>POST TO</div>
          <div style={{ display: 'flex', gap: 8, marginBottom: audience === 'groups' ? 14 : 0 }}>
            {audienceOptions.map(opt => (
              <div key={opt.id} onClick={() => setAudience(opt.id)} style={{ flex: 1, padding: '10px 6px', textAlign: 'center', borderRadius: 8, cursor: 'pointer', background: audience === opt.id ? C.accentDim : 'transparent', border: `1.5px solid ${audience === opt.id ? C.accent : C.border}` }}>
                <div style={{ fontSize: 16, marginBottom: 4 }}>{opt.icon}</div>
                <div style={{ fontSize: 12, fontWeight: audience === opt.id ? 700 : 400, color: audience === opt.id ? C.accent : C.ts }}>{opt.label}</div>
              </div>
            ))}
          </div>

          {audience === 'company' && (
            <div style={{ color: C.ts, fontSize: 12, lineHeight: 1.4 }}>Visible to every member of {companyName} — this is your company's shared, all-members feed.</div>
          )}
          {audience === 'public' && (
            <div style={{ color: C.ts, fontSize: 12, lineHeight: 1.4 }}>Visible to every company on Pro Force Secure — not private to yours.</div>
          )}
          {audience === 'groups' && (
            <div>
              ...
              {closedGroups && closedGroups.length > 0 && (
                <div style={{ color: selectedGroupIds.length ? C.ts : C.danger, fontSize: 11, marginTop: 10 }}>
                  {selectedGroupIds.length ? `Posting to ${selectedGroupIds.length} group${selectedGroupIds.length > 1 ? 's' : ''} — select as many as apply` : 'Select at least one group to post to'}
                </div>
              )}
            </div>
          )}
        ...
        <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 10, padding: '12px 14px', display: 'flex', gap: 10 }}>
          ...
          <span style={{ color: C.ts, fontSize: 12, lineHeight: 1.4 }}>This post will be end-to-end encrypted. Only members of the selected audience can read it.</span>
        </div>
      </div>
...
const GroupDetail = ({ nav }) => {
  const { C, selectedGroup } = useTheme();
      ...
      <div style={{ flex: 1, overflowY: 'auto', paddingBottom: 70 }}>
        {tab === 'posts' && (
          <>
            ...
            {posts && posts.map(p => (
              <div key={p.id} onClick={() => nav('post_detail')} style={{ padding: '14px 16px', borderBottom: `1px solid ${C.border}`, cursor: 'pointer' }}>
                <div style={{ display: 'flex', gap: 10, marginBottom: 10 }}><Avatar letter={(p.author_name || 'M')[0]} size={36} /><div><div style={{ color: C.tp, fontSize: 13, fontWeight: 700 }}>{p.author_name}</div><div style={{ color: C.ts, fontSize: 11 }}>{timeAgo(p.created_at)}</div></div></div>
        ...
        {tab === 'members' && (
          <div style={{ padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: 14 }}>
            <div style={{ color: C.ts2, fontSize: 12, fontStyle: 'italic', marginBottom: 4 }}>Member list wiring is next — showing representative data for now.</div>
